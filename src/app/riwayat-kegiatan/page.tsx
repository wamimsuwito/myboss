...
import { ArrowLeft, Loader2, History, Calendar as CalendarIcon, Camera, Clock, FilterX } from 'lucide-react';
...
export default function RiwayatKegiatanPage() {
    ...
    useEffect(() => {
        if (!userInfo) return; // Wait until userInfo is loaded

        const fetchHistory = async () => {
            setIsLoading(true);
            try {
                let q;
                const canViewAll = userInfo.jabatan?.toUpperCase() === 'HRD PUSAT';

                if (canViewAll) {
                    // HRD can see all activities
                    ...
                    // If no user ID, don't fetch anything
                    setHistory([]);
                    setIsLoading(false);
                    return;
                }
    ...
    const PhotoWithTimestamp = ({ photo, timestamp, label, formatStr = 'dd MMM, HH:mm' }: { photo?: string, timestamp?: any, label: string, formatStr?: string }) => {
